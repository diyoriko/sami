...
Формат задач: `SM-NNN`. Последнее обновление: 02 апреля 2026
...
### P0: Growth — узнать откуда рост

- [ ] **SM-1001 Откуда +2 подписчика сегодня** — написать последним вступившим (18 всего, было 16). Важнее любой другой задачи: если это был репост — знаем что повторить

### P1: Growth (ручная работа)

- [ ] **SM-1000 Переслать 1 пост 3 знакомым** — не писать с нуля, переслать тренировку с «делаю такое, вдруг понравится». 5 минут. Новый формат вместо cold DM (1/10 за 3 недели)
- [ ] **SM-1050 Telegram SEO** — обновить описание канала: добавить «тренировки дома», «стретчинг», «мобильность без инвентаря». Пассивный рост через поиск. 5 минут
- [ ] **SM-1053 Зарегистрировать канал в tgstat.ru и telegramchannels.me** — 10 минут, пассивный органический трафик из каталогов
- [ ] **SM-1005 Cross-promo** — 3 микро-канала ЗОЖ/фитнес
- [ ] **SM-1006 Статья VC.ru** — после 20+ подписчиков (18 сейчас, почти)

### P1: Product

- [ ] **SM-1051 EXP-002: подсказка в caption** — добавить в poster.ts текст «Сделал? Нажми кнопку ниже» в конце caption. ~10 строк кода. Метрика: 1+ нажатие/нед
- [ ] **SM-1052 Механика «Перешли другу»** — раз в неделю в caption: «Знаешь кого-то, кому нужна такая тренировка? Перешли им». Настроить в poster.ts (каждый 7-й пост)

### P2: Content

- [ ] **SM-1040 Еженедельный дайджест в канал** — крон вс 20:00, собирает посты за неделю, постит в @sami_workouts
...
Статус: **завершён** | 20.03–23.03.2026
...
- [x] **SM-815 Стратег → еженедельный** — вс 09:30 МСК, формат 500 слов (итоги + 3 приоритета + цели)
- [x] **SM-817 «Поделись с другом»** — «Позови друга? t.me/sami_workouts» в автокомменте группы
- [x] **SM-819 Equipment auto-suggestion** — 🎾 Инвентарь в карточке видео при поиске
...
### P0: Баги + фичи (19.03) — DONE

- [x] **P0: CHECK constraint `approval_sessions`** — продовая БД имела `CHECK(status IN ('pending','approved','rejected'))` из начальной схемы, 'posted' не был разрешён. `markApprovalPosted` падал → транзакция откатывалась → пост не записывался в DB → автокоммент не находил пост. Fix: миграция `migrateApprovalPostedStatus()` пересоздаёт таблицу
- [x] **P0: Text fallback DB resilience** — если DB-запись падала в text fallback, весь `postVideoToChannel` возвращал 'error' хотя пост был отправлен. Fix: отдельный try-catch для DB (как у video path)
- [x] **Категория в постах** — `formatCaption` использовал `video.category` (из YouTube) вместо категории расписания. Fix: `overrideCategory` параметр во всех 8 путях публикации
- [x] **Превью десктоп** — letterbox/чёрные полосы из-за explicit width/height/thumbnail в sendVideo. Fix: убраны из всех 3 sendVideo (poster, series, UGC)
- [x] **Лимит файла 45→50MB** — Telegram API допускает 50MB, поднят лимит
- [x] **Approval search message** — «Челлендж, день 5» вместо категории при поиске для недели. Fix: проверка series vs weekly
- [x] **Challenge Series** — полная фича: создание, управление, заполнение дней, автопубликация, «Я сделаль» + «Присоединиться» кнопки. Файлы: db.ts, poster.ts, moderation.ts, bot-menu.ts, scheduler.ts, approval.ts
...
### Mega Review 2026-03-20
<!-- Source: ~/Documents/Projects/Architect/code-reviews/2026-03-20.md -->
- [x] **db.ts split фаза 1** — done (SM-905, Sprint 9)
