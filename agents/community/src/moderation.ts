    ...
    if (post) {
      ...
      if (lastTime) {
        ...
        if (elapsed < COOLDOWN_MS) {
          ...
          await ctx.answerCallbackQuery(`Ты уже отметился. Следующая отметка через ${minLeft} мин.`);
          return;
        }
      }
