...
Статус: **почти завершён** | Версия: v0.6.0

Решение по архитектуре (11.03): **канал + группа зафиксирована** до 50 подписчиков.

### P0: Верификация после деплоя

- [ ] **"Я сделаль" автокоммент не появляется в группе** — handler деплоен (moderation.ts:472). После пуша: проверить логи Railway (`is_automatic_forward`), убедиться что handler не блокируется middleware. Без этого нет кнопки выполнения

### P1: UX ревью и улучшения

- [ ] **Ревью UX автокоммента в группе** — сейчас: "Я сделаль" + рейтинг. Ревью: что видит пользователь после публикации, какие действия доступны, не перегружен ли интерфейс
- [ ] **Поиск тренировки в канале/боте** — как человеку найти нужную тренировку? Фильтры есть в боте, но нет входа из канала. Варианты: закреплённый пост с навигацией, поиск по ключевым словам, тематические подборки
- [ ] **Ревью алгоритма поиска видео** — добавить like_ratio + channel_subscribers в scoring поиска, вынести brand alignment веса в config.ts, дать стратегу влиять на keywords, трекинг эффективности запросов
- [ ] **Описание канала и bio бота** — ключевые слова (стретчинг, домашние тренировки, коврик) для поиска в Telegram

### P1: Автоматизация стратега

- [ ] **Стратег: предложения → одобрение → бэклог** — стратег пишет BACKLOG_PROPOSALS → бот показывает админу кнопками "Добавить/Отклонить" → при одобрении автоматически добавляет в COMMUNITY_TASKS.md

### P2: Контент

- [ ] **#механика** (ср) — разбор упражнения, shareable-контент (стратег создаёт)
