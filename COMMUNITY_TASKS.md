...
- [ ] **Очеловечить приветствие бота** — сейчас "Привет! Я бот Sami." Нужно: называть себя "Ботик Сами", тёплый тон, кратко объяснить что умеет
- [ ] **Кнопки сложности обрезаются** — "💎 На...ающий", "💎💎💎 ...нутый". Убрать множественные эмодзи, сделать по 1 в ряд или сократить текст (Легко / Средне / Сложно)
- [ ] **UGC flow: недостающие параметры** — бот не спрашивает группы мышц ни для видеофайла, ни для YouTube-ссылки. Ревью всего flow: какие поля нужны для полноценного поста (категория, сложность, мышцы, длительность, инвентарь). Для видеофайлов длительность можно вытащить автоматически
- [ ] **UGC YouTube: Error 153 при одобрении** — "Video player configuration error" в Telegram mini app при открытии YouTube-ссылки после одобрения. Проверить: скачивается ли видео через yt-dlp или постится как ссылка, корректен ли формат поста
- [ ] **Поиск видео: дублирует сообщение** — "Ищу видео на 2026-03-12..." отправляется дважды. Найти и убрать дубль
- [ ] **Убрать "Опубликовать" из persistent menu** — вместо пакетной публикации: inline-кнопка "Опубликовать" появляется рядом с "Выбрано" при выборе конкретного видео из поиска. Также убрать "Сбросить выбор" из меню
- [ ] **"Другое" в поиске: "Сессия не найдена"** — при нажатии "Другое" в категории Восстановление (и возможно других новых категориях). Approval session не создаётся или не находится для новых категорий после v0.6.0
- [ ] **"Я сделаль": Loading и зависание** — нажатие кнопки "Я сделаль" в discussion group показывает "Loading..." и ничего не происходит. Счётчик "Сделали: 0" в теле поста не обновляется. Разобраться: (1) callback `done:` или `done_msg:` — какой вызывается и почему зависает, (2) может ли бот вообще редактировать caption канального поста из discussion group, (3) защита от множественного нажатия — cooldown не срабатывает или не показывает сообщение
- [ ] **Рейтинг в теле поста некликабельный** — "★ 6.4 из 10" в caption поста должен быть интерактивным (кнопка или ссылка) с popup-объяснением формулы рейтинга. Сейчас это просто текст. Inline-кнопки убраны из канала (Sprint 5) — нужно другое решение: например кнопка в автокомменте или текстовая команда
- [ ] **Статус: очередь показывает невыбранные видео** — в "Очередь" отображаются все результаты поиска (⏳ pending), а не только выбранные (✅ approved). Показывать только одобренные видео, или чётко разделять "выбрано" / "ожидает выбора"
