...
### P0: Критические баги — бот не может постить

Без решения этих задач канал мёртв. Всё остальное блокировано.

- [x] **Публикация YouTube-видео не работает** — хардкод 3 старых категорий в "Опубликовать" (bot-menu.ts). Фикс: CATEGORIES из shared.ts, показываем только выбранные
- [x] **UGC "Тест 3" не опубликовалась** — не было трекинга публикации. Фикс: добавлен `published_at` в ugc_submissions, ставится после успешной отправки в канал
- [x] **"Я сделаль" автокоммент не появляется в группе** — handler падал если пост не в DB. Фикс: fallback кнопка "Я сделаль" даже для незарегистрированных постов
- [x] **Health dashboard "Недоступен"** — FK migration crash. Hotfix `2f94836`: pragma foreign_keys=OFF перед rebuild. Бот поднялся

### P1: UX баги и улучшения

- [x] **UX сообщения при публикации** — показывает все категории с "не выбрано". Фикс: показывать только выбранные + название видео
- [x] **"Мои тренировки": показывать только опубликованные** — добавлен `published_at`, фильтр по нему. "Одобрена" != "опубликована"
- [ ] **Ревью UX автокоммента в группе** — что видит пользователь после публикации, какие действия доступны, не перегружен ли интерфейс
- [ ] **Описание канала и bio бота** — ключевые слова (стретчинг, домашние тренировки, коврик) для поиска в Telegram
- [ ] **Формат автора в постах** — сейчас "Автор: Том Меррик" (ссылка на канал). Нужно: "Автор: Том Меррик, 📎 [YouTube](ссылка)"

### P1: Качество контента

- [ ] **Ревью алгоритма поиска видео** — like_ratio + channel_subscribers в scoring, brand alignment веса в config.ts, стратегу влиять на keywords, трекинг эффективности запросов
- [ ] **Поиск тренировки в канале/боте** — как человеку найти нужную тренировку? Фильтры есть, но нет входа из канала. Варианты: закреплённый пост, поиск по словам, подборки

### P2: Автоматизация

- [ ] **Стратег: предложения → одобрение → бэклог** — BACKLOG_PROPOSALS → кнопки "Добавить/Отклонить" → автоматически в COMMUNITY_TASKS.md
...
- [ ] MarkdownV2 — низкий приоритет, высокий риск регрессий
- [ ] Лендинг Sami (Figma готов)
- [ ] Ручной посев — 10 личных приглашений
