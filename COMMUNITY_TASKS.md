...
Статус: **запланирован** | Фокус: постить классные видео, стратег исполняет, UX канала
...
### P1: Стратег v2 — действия с одобрением

Цель: стратег предлагает конкретные действия, админ одобряет кнопкой, бот выполняет.

- [ ] **COMMUNITY_PACKET v2** — добавить поле `actions[]` с типизированными действиями:
  - `create_poll` — создать опрос в группе
  - `update_welcome` — изменить welcome-текст
  - `limit_posts` — изменить лимит постов/день
  - `send_digest` — отправить weekly digest подписчикам
- [ ] **DM с кнопками** — стратег предлагает действие → бот показывает админу с кнопками "Выполнить / Отклонить"
- [ ] **Исполнение** — бот выполняет одобренные действия (опрос, welcome-текст и т.д.)
- [ ] **Feedback loop** — бот записывает результат (опрос создан, N голосов) → стратег видит в следующем цикле

### P1: UX постов в канале

- [ ] **Кнопка "Закрепить"** — рядом с "Я сделаль", сохраняет видео в закреплённые
- [ ] **Рейтинг: пояснение** — при нажатии на рейтинг показать popup с объяснением формулы
- [ ] **"Я сделаль" после просмотра** — исследовать: Telegram API не даёт событие просмотра. Варианты: таймер, честная система
...
- [x] **"Пост не найден" при "Я сделаль"** — getPostByMessageId не находит пост (message_id отличается при просмотре из группы). Fix: fallback на поиск по video_id
- [x] **Счётчик "Сделали: N" в теле поста** — добавлен в caption, обновляется при каждом клике "Я сделаль"
- [ ] **Комментарии в канале** — комментарии пропали. Проверить: канал @sami_workouts должен быть привязан к группе "Сами Daily" через настройки канала → Discussion
- [ ] **Капча при вступлении** — работает только для группы, не для канала. Решить: верификация подписчиков канала или перенаправление в группу
- [ ] **HTML entities в статусе** — `&quot;` в названиях видео не декодируются
...
### P2: Фильтрация видео через бота
...
### P2: Социальные профили участников
