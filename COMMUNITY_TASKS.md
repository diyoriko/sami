...
- [ ] **UGC не публикуется после одобрения** — одобрение меняет статус, но не постит в канал. Нужно: при approve → скачать видео → опубликовать в канал с caption (автор, категория, сложность) → записать в posts
...
- [ ] **Persistent menu не обновляется** — новые кнопки (Фильтры, Сохранённое, Профиль) появляются только после /start
...
### P1: Продвинутая модерация группы

Источник: @merskie_dela (#10, #138) — антиспам-система с 99% эффективностью.
Контекст: текущая капча + фильтр ссылок недостаточны. Спамеры проходят капчу вручную.

- [ ] **Content-based фильтрация** — анализ текста сообщений на спам-паттерны (крипта, казино, заработок) с автоудалением. Расширить SPAM_PATTERNS в moderation.ts
- [ ] **Cooldown для новичков** — ограничить частоту сообщений для новых участников (первые 24ч: max 1 сообщение в минуту)
- [ ] **Антифлуд** — автомут при 5+ сообщениях за 30 секунд от одного пользователя
- [ ] **Репутационная система** — новички с 0 выполнений: усиленный фильтр. Практики/наставники: ослабленный. Интеграция с getMemberLevel()
- [ ] **Логирование удалений** — таблица moderation_log: что удалено, почему, кем (бот/админ). Для аналитики и тюнинга
- [ ] **Стоп-лист фраз** — динамический список в bot_config (стратег может предлагать обновления через actions)
- [ ] **Ночной режим** — усиленная модерация в часы с 00:00 до 07:00 МСК (спам чаще ночью)
