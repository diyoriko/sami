...
- [ ] **"Мои тренировки": показывать все статусы** — сейчас только опубликованные (`published_at IS NOT NULL`). Пользователь не видит свои pending/approved заявки. Нужно показывать все с пометкой статуса (⏳ на модерации, ✅ опубликовано, ❌ отклонено)
- [ ] **UGC: предупреждение о дубликате YouTube-ссылки** — если пользователь отправляет ту же ссылку, предупредить что такое видео уже было предложено
